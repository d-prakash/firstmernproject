                ...
                <div className="contact-form">
                    <form id="contact-form" onSubmit={handleSubmit}>
                        <Row>
                            ...
                            <Col md={6}>
                                <div className="single-form form-group">
                                    <input type="email" name="email" placeholder="Your Email" value={form.email || ""} onChange={(e)=> setField("email",e.target.value) } data-error="Valid email is required." />
                                    <div className="feedback-error">{errors.email}</div>
                                </div> 
                            </Col>
                            <Col md={6}>
                                <div className="single-form form-group">
                                    <input type="text" name="subject" placeholder="Subject" data-error="Subject is required." value={form.subject || ""}  onChange={(e)=> setField("subject",e.target.value) }/>
                                    <div className="feedback-error">{errors.subject}</div>
                                </div> 
                            </Col>
                            <Col md={6}>
                                <div className="single-form form-group">
                                    <input type="text" name="phone" placeholder="Phone" data-error="Phone is required."  value={form.phone || ""} onChange={(e)=> setField("phone",e.target.value) } />
                                    <div className="feedback-error">{errors.phone}</div>
                                </div> 
                            </Col>
                            <Col md={12}>
                                <div className="single-form form-group">
                                    <textarea placeholder="Your Mesaage" name="message" data-error="Please, leave us a message." value={form.message || ""} onChange={(e)=> setField("message",e.target.value) } ></textarea>
                                    <div className="feedback-error">{errors.message}</div>
                                </div> 
                            </Col>
                            {isSubmit && <span className="form-message">Oops! An error occuered and Your message could nont be sent.</span>}
                            <Col md={12}>
